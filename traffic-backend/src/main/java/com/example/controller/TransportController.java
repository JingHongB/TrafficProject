...
public class TransportController {
    ...
    @Operation(summary = "获取两点之间路径")
    @GetMapping("/getPath")
    public RestBean<String> getPath(){

        try {
            String pathStr = pathService.getPath();
            return RestBean.success(pathStr);
        } catch (Exception e) {
            return RestBean.failure(500, "获取两点之间路径失败");
        }

    }

    /*@Operation(summary = "让车辆沿路径行驶")
    @PostMapping("/driving")*/
